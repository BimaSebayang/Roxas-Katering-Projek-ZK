...
JpaRepository<TableMenu, TableMenuPk>{
 ...
 @Modifying
	...
	@Query("select a.menuCode, a.menuName, a.notes from TableMenu a "
			+ " where a.status = '1' "
			+ " and a.projekCode = :projekCode "
			+ " and (upper(a.menuCode) like upper(:search) "
			+ " or "
			+ " upper(a.menuName) like upper(:search) "
			+ " or "
			+ " upper(a.notes) like upper(:search) )")
	public List<Object[]> getAllMenusInCurrentProjek(@Param("projekCode") String projekCode, @Param("search") String search);
	//KAtering projek --end
