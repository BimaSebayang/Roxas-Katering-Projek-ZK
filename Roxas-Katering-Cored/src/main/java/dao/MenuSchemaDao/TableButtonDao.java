...
public interface TableButtonDao extends
	...
	//KAtering projek --start
	@Query("select a.buttonCode, a.buttonName, a.notes from TableButton a where "
			+ " a.status = '1' "
			+ " and "
			+ " a.projekCode = :projekCode "
			+ " and a.buttonCode not in (select b.menuButton from TableConfigurationMenu b where "
			+ " b.userId = :userId and b.menuCode = :menuCode ) "
			+ " and "
			+ " (upper(a.buttonCode) like upper(:search) "
			+ "  or "
			+ "  upper(a.buttonName) like upper(:search) "
			+ "  or "
			+ "  upper(a.notes) like upper(:search) )")
	public List<Object[]> getAllButtonInCurrentUserIdAndMenu(@Param("projekCode") String projekCode, @Param("menuCode") String menuCode, 
			@Param("userId") String userId, @Param("search") String search);
	//KAtering projek --end
