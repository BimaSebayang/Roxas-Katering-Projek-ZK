...
public class TableUserCustomDaoSvcImpl implements TableUserCustomDaoSvc {
	...
	public Map<String, Object> loginUser(String userId, String password) {
		...
		if(count==0) {
			...
			map.put("statement", "User Belum Terdaftar");
		}
		else {
		String sqlCount2 = " select count(a.USER_ID) as existing_userId from MENU.TABLE_USER_PEGANTI a "
	            + " where a.USER_ID = '"+userId+"' and a.STATUS = '1' ";
		int count2 = (int) em.createNativeQuery(sqlCount2).getSingleResult();
		if(count2==0){
			map.put("countUser", count2);
			map.put("userId", new TableUser());
			map.put("result", false);
			map.put("statement", "User Sudah Tidak Aktif");
		}
	    else{
	    	
	    	String sqlCount3 = " select count(a.USER_ID) as existing_userId from MENU.TABLE_USER_PEGANTI a "
		            + " where a.USER_ID = '"+userId+"' and a.PASSWORD='"+password+"' ";
			int count3 = (int) em.createNativeQuery(sqlCount3).getSingleResult();
	    
		if(count3==0){
			map.put("countUser", count3);
			map.put("userId", new TableUser());
			map.put("result", false);
			map.put("statement", "Password Masih Salah");
		}	
